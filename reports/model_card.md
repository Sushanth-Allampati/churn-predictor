...
**URL**      : https://churn-predictor-api-gonj.onrender.com
**Docs**     : https://churn-predictor-api-gonj.onrender.com/docs

### Performance (measured on live server)
- Warm prediction latency : ~150-200ms
- Cold start time         : 30-60 seconds (free tier spins down after 15min inactivity)
- Batch of 50 customers   : ~500ms warm

### Known limitations
- Free tier has cold start delay — first request after inactivity is slow
- No persistent storage — model is baked into the container image
- Single worker — not suitable for high concurrency